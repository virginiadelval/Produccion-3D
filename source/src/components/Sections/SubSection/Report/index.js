...
import reportePdfIcon from 'img/reporte_parcelario_pdf.png'

const Item = ({ smp, address, state, onClick }) => {
  return (
    <Card sx={styles.card}>
      <Typography variant="subtitle1">Catastro: {smp}:</Typography>
      {state === 'ready' && (
        <Tooltip title="descarga el Reporte Parcelario (PDF)">
          <IconButton onClick={() => onClick('PDF')} sx={styles.icon}>
            <img
              src={reportePdfIcon}
              width="24px"
              alt="Icono reporte PDF"
            />
          </IconButton>
        </Tooltip>
      )}
